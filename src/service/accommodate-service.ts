...
import { AccommodateRequest, AccommodateResponse, toAccommodateResponse } from "../model/accommodate-model";
...
export class AccommodateService {
  ...
  static async storeColleger(request: AccommodateRequest): Promise<any> {
    const requestBody: AccommodateRequest = Validation.validate(
      AccommodateValidation.accommodateStoreValidation,
      request
    );
  ...
  static async destroyAccommodateByAccommodateId(accommodateId: number): Promise<AccommodateResponse>{
    const [accommodate] = await prisma.$transaction([
